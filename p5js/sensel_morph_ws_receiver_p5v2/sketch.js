/**
 * Sensel Morph WebSocket receiver for p5.js 2.x.
...
function drawLabelsImage() {
  ...
  drawImageWithAlpha(labelsImage, 0, 0, PRESSURE_DISPLAY_W, PRESSURE_DISPLAY_H, LABEL_ALPHA / 255.0);
}
...
function drawHeader() {
  ...
  textSize(16);
  text("Sensel Morph WebSocket Receiver (p5 v2)", 16, 26);
...
function drawImageWithAlpha(img, x, y, w, h, alpha01) {
  const ctx = drawingContext;
  ctx.save();
  ctx.globalAlpha = constrain(alpha01, 0, 1);
  image(img, x, y, w, h);
  ctx.restore();
}
