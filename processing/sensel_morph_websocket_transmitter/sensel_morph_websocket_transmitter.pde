// Sensel Morph WebSocket Transmitter
...
// streams; and broadcasts them over WebSocket. This app does not require
// Python or the Sensel SDK.
//
...
// calibration, and display sampling while the sketch is running. Press c to
// toggle calibration when a matching
...
void setup() {
  ...
  surface.setTitle("Sensel Morph WebSocket Transmitter");
  noSmooth();
  frameRate(60);
...
void ensureWebSocketServer() {
  ...
  try {
    ws = new MorphWebSocketServer(config.host, config.port);
    ws.start();
    statusLine = "WebSocket ws://" + config.host + ":" + config.port;
  } catch (Exception e) {
    ws = null;
    statusLine = "WebSocket start failed: " + e.getMessage();
    println(statusLine);
  }
}
...
void recordingLoop() {
  try {
    ...
    statusLine = "playing " + recordingSource.sourceName() + " on ws://" + config.host + ":" + config.port;
...
void sendStatus(String portName) {
  if (ws == null) return;
  ws.broadcastText("{"
    + wsJsonPair("type", "status") + ","
    + wsJsonPair("device", portName) + ","
    + wsJsonPair("serial_number", deviceSerial) + ","
    + wsJsonPair("pressure", config.pressure) + ","
    + wsJsonPair("labels", config.labels) + ","
    + wsJsonPair("contacts", config.contacts) + ","
    + wsJsonPair("accelerometer", true) + ","
    + wsJsonPair("pressure_res", config.pressureRes) + ","
    + wsJsonPair("label_res", config.labelRes) + ","
    + wsJsonPair("pressure_type", config.pressureType) + ","
    + wsJsonPair("calibrated", calibrationActive()) + ","
    + wsJsonPair("pressure_normalize", false) + ","
    + wsJsonPair("rle", config.rle) + ","
    + wsJsonPair("force_scale", config.forceScale) + ","
    + wsJsonPair("clients", ws.clientCount())
    + "}");
...
void sendLiveFrame(LiveFrame frame) {
  ...
  if (config.pressure && frame.pressureBlob != null) {
    ws.broadcastBinary(wsRasterPacket("SMPR", 1, frame.frameId, frame.timestamp, frame.pressureWidth, frame.pressureHeight, frame.pressureBitDepth, pressureRasterFlags(), frame.pressureMax, frame.pressureBlob));
  }

  if (config.labels && frame.labelBlob != null) {
    ws.broadcastBinary(wsRasterPacket("SMLB", 2, frame.frameId, frame.timestamp, frame.labelWidth, frame.labelHeight, 8, labelRasterFlags(), 0.0, frame.labelBlob));
  }
  ...
  if (frame.accel != null) {
    ws.broadcastText("{"
      + wsJsonPair("address", "/sensel_morph/accelerometer") + ","
      + wsJsonPair("frame_id", frame.frameId) + ","
      + wsJsonPair("x", frame.accel.x) + ","
      + wsJsonPair("y", frame.accel.y) + ","
      + wsJsonPair("z", frame.accel.z) + ","
      + wsJsonPair("x_g", frame.accel.x / config.accelCountsPerG) + ","
      + wsJsonPair("y_g", frame.accel.y / config.accelCountsPerG) + ","
      + wsJsonPair("z_g", frame.accel.z / config.accelCountsPerG)
      + "}");
  }
...
int pressureRasterFlags() {
  int flags = calibrationActive() ? 0x01 : 0;
  if (config.rle) flags |= 0x04;
  return flags;
}

int labelRasterFlags() {
  return config.rle ? 0x04 : 0;
}
