...
void drawSettingsUi() {
  ...
  text("transmit streams", x + 12, y); y += 18;
  drawUiCheckbox("pressure", config.pressure, x + 16, y); y += UI_ROW;
  drawUiCheckbox("labels", config.labels, x + 16, y); y += UI_ROW;
  drawUiCheckbox("contacts", config.contacts, x + 16, y); y += UI_ROW + 8;
...
void drawLocalViewControls(int x, int y) {
  ...
  drawUiRadio("nearest", surfaceSamplingModeIndex == SURFACE_SAMPLING_NEAREST, x, y); y += UI_ROW;
  drawUiRadio("linear", surfaceSamplingModeIndex == SURFACE_SAMPLING_LINEAR, x, y);
}
