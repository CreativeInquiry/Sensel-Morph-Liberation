...
// Experimental pressure+contacts/no-labels ellipse path. If this is false, the
// app falls back to firmware ellipses whenever labels are unavailable.
final boolean ENABLE_HYBRID_PRESSURE_BBOX_ELLIPSES = true;
final int HYBRID_PRESSURE_BBOX_EXPAND_CELLS = 1;
 ...
 * Contacts-only mode remains lightweight. Pressure+labels gets the full
 * label-derived ellipse fit; pressure+contacts/no-labels gets the hybrid bbox
 * fit for isolated contacts and firmware fallback for complex overlaps.
 */
void prepareOutputRasters(LiveFrame frame, MorphSettings settings) {
  boolean displayPlayback = settings.source.equals("recording");
  if (frame.pressure != null && (needsPressureOutput(settings) || displayPlayback)) {
  ...
  if (frame.labels != null && (needsLabelOutput(settings) || displayPlayback)) {
  ...
  if (frame.contacts != null && (needsContactOutput(settings) || displayPlayback)) {
    if (frame.pressure != null && frame.labels != null) {
      frame.rasterEllipses = rasterEllipsesByLabel(frame.pressure, frame.labels);
    } else if (shouldUseHybridPressureBboxEllipses(frame, settings, displayPlayback)) {
      frame.rasterEllipses = hybridEllipsesByFirmwareBbox(frame.pressure, frame.contacts);
    }
...
boolean shouldUseHybridPressureBboxEllipses(LiveFrame frame, MorphSettings settings, boolean displayPlayback) {
  if (!ENABLE_HYBRID_PRESSURE_BBOX_ELLIPSES) return false;
  if (frame.pressure == null || frame.contacts == null || frame.labels != null) return false;
  return (needsContactOutput(settings) || displayPlayback) && (needsPressureOutput(settings) || displayPlayback);
}

/**
 * Hybrid pressure+bbox ellipses for pressure+contacts/no-labels capture.
 *
 * Isolated firmware bboxes get a fresh pressure-weighted moment ellipse. Bboxes
 * that overlap another firmware bbox intentionally emit no raster ellipse, so
 * the existing contactEllipse() fallback keeps the firmware ellipse.
 */
HashMap<Integer, RasterEllipse> hybridEllipsesByFirmwareBbox(DecodedPressure pressureFrame, DecodedContactsFrame contactsFrame) {
  HashMap<Integer, RasterEllipse> ellipses = new HashMap<Integer, RasterEllipse>();
  if (pressureFrame == null || contactsFrame == null || contactsFrame.contacts == null) return ellipses;
  if ((contactsFrame.contactMask & 0x04) == 0) return ellipses;
  int w = pressureFrame.grid.cols;
  int h = pressureFrame.grid.rows;
  for (SenselContact c : contactsFrame.contacts) {
    if (!hasUsableFirmwareBbox(c) || !isIsolatedFirmwareBbox(c, contactsFrame.contacts, w, h)) {
      continue;
    }
    int[] bounds = firmwareBboxPixelBounds(c, w, h, HYBRID_PRESSURE_BBOX_EXPAND_CELLS);
    RasterEllipse fit = fitPressureMomentEllipseInBounds(pressureFrame.values, w, h, bounds[0], bounds[1], bounds[2], bounds[3]);
    if (fit != null) {
      ellipses.put(Integer.valueOf(c.id), fit);
    }
  }
  return ellipses;
}

boolean isIsolatedFirmwareBbox(SenselContact c, SenselContact[] contacts, int w, int h) {
  int[] a = firmwareBboxPixelBounds(c, w, h, 0);
  for (SenselContact other : contacts) {
    if (other == c || !hasUsableFirmwareBbox(other)) continue;
    int[] b = firmwareBboxPixelBounds(other, w, h, 0);
    if (pixelBoundsOverlap(a, b)) {
      return false;
    }
  }
  return true;
...
boolean hasUsableFirmwareBbox(SenselContact c) {
  // The firmware can report a point-like bbox during early/light contact
  // frames: min == max, area == 0, but the point is still a useful seed for the
  // hybrid pressure+bbox fit once expanded by a cell.
  return c.maxX >= c.minX && c.maxY >= c.minY;
}

boolean pixelBoundsOverlap(int[] a, int[] b) {
  return a[0] < b[2] && a[2] > b[0] && a[1] < b[3] && a[3] > b[1];
}

int[] firmwareBboxPixelBounds(SenselContact c, int w, int h, int expandCells) {
  int x0 = floor(c.minX / ACTIVE_W_MM * w - 0.5) - expandCells;
  int y0 = floor(c.minY / ACTIVE_H_MM * h - 0.5) - expandCells;
  int x1 = ceil(c.maxX / ACTIVE_W_MM * w + 0.5) + expandCells;
  int y1 = ceil(c.maxY / ACTIVE_H_MM * h + 0.5) + expandCells;
  return new int[] {
    constrain(x0, 0, w),
    constrain(y0, 0, h),
    constrain(x1, 0, w),
...
RasterEllipse fitPressureMomentEllipseInBounds(int[] pressure, int w, int h, int x0, int y0, int x1, int y1) {
  if (pressure == null || x1 <= x0 || y1 <= y0) return null;
  int area = 0;
  double total = 0;
  double sx = 0;
  double sy = 0;
