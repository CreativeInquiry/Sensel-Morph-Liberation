...
void acceptLiveFrameForDisplay(LiveFrame frame) {
  ...
  if (needsPressureOutput(config) && frame.pressureBlob != null) {
  ...
  if (frame.labelBlob != null) {
    labelsImage = needsLabelOutput(config) ? labelsImageFromBlob(frame.labelBlob, frame.labelWidth, frame.labelHeight) : null;
  ...
  if (needsContactOutput(config) && frame.contacts != null) {
  ...
  } else if (!needsContactOutput(config)) {
...
void drawPressureBuffer(PImage pressure) {
  ...
  if (pressure != null) {
    ...
    applyPressureDisplaySampling(pressureBuffer, pressure);
    pressureBuffer.image(pressure, r.x, r.y, r.w, r.h);
    pressureBuffer.resetShader();
    if (pressureSamplingWarmupFramesRemaining > 0) {
      pressureSamplingWarmupFramesRemaining--;
    }
  }
...
void applyPressureDisplaySampling(PGraphics pg, PImage pressure) {
  if (effectivePressureSamplingMode() == SURFACE_SAMPLING_BICUBIC && bicubicPressureShader != null) {
    bicubicPressureShader.set("texelSize", 1.0 / pressure.width, 1.0 / pressure.height);
    pg.shader(bicubicPressureShader);
    return;
  }
...
String pressureSamplingName() {
  if (pressureSamplingWarmupFramesRemaining > 0) {
    return "nearest warmup";
  }
  if (surfaceSamplingModeIndex == SURFACE_SAMPLING_BICUBIC) return "bicubic";
  return "nearest";
}

int effectivePressureSamplingMode() {
  if (pressureSamplingWarmupFramesRemaining > 0) {
    return SURFACE_SAMPLING_NEAREST;
  }
  return surfaceSamplingModeIndex;
}
...
void drawHud() {
  ...
  textSize(12);
  int y = 16;
  text("sensel_morph_syphon_osc_transmitter", 16, y); y += 16;
  text(statusLine, 16, y); y += 16;
  text("serial_number: " + deviceSerial, 16, y); y += 16;
  text(syphonStatus, 16, y); y += 16;
  y += 4;
  ...
  text("sampling: " + pressureSamplingName(), 16, y); y += 16;
  y += 4;
  if (accelFrame.valid) {
